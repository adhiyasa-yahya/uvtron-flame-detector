...
void setup()
{
  ...
  lastEEPROMData = EEPROM.read(dataAddress); // lastEEPROMData baca dataAddress
  condition = lastEEPROMData;                // Baca Condition pake EEPROM
...
void io_poll()                                       
  { 
    ...
    bitWrite( au16data[0], B00000100, digitalRead(7));
...
{  
  ...
  {
    EEPROM.update(dataAddress, 1);
    condition = 1;
  }

  if (condition == 0)
   {                                  // Kondisi stand by atau tidak ada api
